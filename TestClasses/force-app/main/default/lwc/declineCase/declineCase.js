...
export default class cancelOrder extends LightningElement {
    ...
    get audit() {
        return this.cases.data.fields.Audit_Task_Created__c.value;
    }
    declineCase() {

            if( this.audit === true) 
                alert('Please Complete the Task before closing the case'); 
            else { 
            ...
            updateRecord(record)
                .then(() => {
                    this.dispatchEvent(
                        new ShowToastEvent({
                            title: 'Success',
                            message: 'Case Status updated to Decline',
                            variant: 'success',
                        }),
                    );
