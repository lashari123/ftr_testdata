...
const fields = [AGREEMENT_FIELD,DOCUMENTSTATUS_FIELD,CUSTOMERSIGNED_FIELD,COMPANYSIGNED_FIELD,CONTRACTID_FIELD,CONTRACTFOLDER_FIELD,ACCOUNTCHANNEL_FIELD,RECORDTYPE_FIELD,SPRINGFOLDER_FIELD,SCHEDULETYPE_FIELD];
export default class generateDocument extends LightningElement {
   ...
   newmet(){
    ...
    if(this.companysigned === null && (this.profilenm !== '#CTF Small Business User' || this.custsigned === null)){ 
        ...
        window.open(`https://na11.springcm.com/atlas/doclauncher/eos/CC-${this.agreement}?aid=17662&eos[0].Id=${this.contrid}&eos[0].Id=${this.contrid}&eos[0].System=Salesforce&eos[0].Type=Contract&eos[0].Name=${this.contractfolder}&eos[0].ScmPath=/Accounts/${this.accountchannel}/${this.springfolder}`); 
    }else if(this.profilenm !== '#CTF Small Business User' && this.recordtype ==='Frontier Services Agreement'){ 
        window.open(`https://na11.springcm.com/atlas/doclauncher/eos/${this.agreement}?aid=17662&eos[0].Id=${this.contrid}&eos[0].Id=${this.contrid}&eos[0].System=Salesforce&eos[0].Type=Contract&eos[0].Name=${this.contractfolder}&eos[0].ScmPath=/Accounts/${this.accountchannel}/${this.springfolder}`); 
    }else if(this.profilenm === '#CTF Small Business User'){ 
        window.open(`https://na11.springcm.com/atlas/doclauncher/eos/CC-${this.scheduletype}?aid=17662&eos[0].Id=${this.contrid}&eos[0].System=Salesforce&eos[0].Type=Contract&eos[0].Name=${this.contractfolder}&eos[0].ScmPath=/Accounts/${this.accountchannel}/${this.springfolder}`); 
    }else { 
        window.open(`https://na11.springcm.com/atlas/doclauncher/eos/${this.scheduletype}?aid=17662&eos[0].Id=${this.contrid}&eos[0].System=Salesforce&eos[0].Type=Contract&eos[0].Name=${this.contractfolder}&eos[0].ScmPath=/Accounts/${this.accountchannel}/${this.springfolder}`); 
    }
    }
    }