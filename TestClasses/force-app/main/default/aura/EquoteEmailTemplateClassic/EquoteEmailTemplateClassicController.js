({
    doInit :  function(component, event, helper) {
        ...
        $A.enqueueAction(action3);
          var action99 = component.get("c.getselectOptions");
        var opts = [];
        action99.setCallback(this, function(response) {
            if (response.getState() == "SUCCESS") {
                var allValues = response.getReturnValue();
                for (var i = 0; i < allValues.length; i++) {
                    opts.push(allValues[i].Email);
                }
   ...
   doSave: function(component, event, helper) {
    ...
    changeItrate :function(component, event, helper) {
        var list =[];
  ...
  scriptsLoaded : function(component, event, helper) {
       ...
       $(".select2Class").select2({
           placeholder: "Select Multiple Emails"
       });
	},
    ...
    sendMail: function(component, event, helper) {
        ...
        var emailList = [];
        emailList.push(getEmail);
        if(selectedSkills != null){
        for(var i = 0 ; i < selectedSkills.length; i++){
            emailList.push(selectedSkills[i]);
        }
        }
        ...
        for(var i = 0 ; i < emailList.length; i++){ 
            if ($A.util.isEmpty(emailList[i]) || !emailList[i].includes("@")) {
                alert('Please Enter valid Email Address');
            } else {
                component.set("v.checkeEmailFormat",'correct');
            }
        }
        var format = component.get("v.checkeEmailFormat");
        if(format == 'correct'){
            helper.sendHelper(component, emailList,attach,name,subject,ownerName,year,emailbody);
        }
    ...
    closeMessage: function(component, event, helper) {
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                var res = response.getReturnValue();
                if(res != null){
                    window.parent.location = res + '/'+ recordId;
                }
            }
        });
